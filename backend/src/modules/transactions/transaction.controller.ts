...
export class TransactionController {
  ...
  createTransaction = async (
    ...
    return reply
      .status(201)
      .send(successResponse("Transaction created successfully"));
  };
  ...
  getTransactions = async (
    ...
    const { data, meta } = await this.transactionService.getTransactions(
      request.query,
    );
    return reply.send(
      successResponse("Transactions fetched successfully", data, meta),
    );
  };
