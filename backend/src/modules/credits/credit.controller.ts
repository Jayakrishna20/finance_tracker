...
export class CreditController {
    ...
    createCredit = async (
        ...
        return reply.status(201).send(successResponse('Credit created successfully'));
    };
    ...
    getCreditById = async (
        ...
        if (!credit) {
            return reply.status(404).send({ success: false, message: 'Credit not found' });
        }
        return reply.send(successResponse('Credit fetched successfully', credit));
    };

    updateCredit = async (
        request: FastifyRequest<{ Params: { id: bigint }; Body: UpdateCreditInput }>,
        reply: FastifyReply
    ) => {
        const { id } = request.params;
